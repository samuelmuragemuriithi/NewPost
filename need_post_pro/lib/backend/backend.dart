...
import 'schema/need_post_record.dart';
...
export 'schema/need_post_record.dart';
...
/// Functions to query NeedPostRecords (as a Stream and as a Future).
Future<int> queryNeedPostRecordCount({
  Query Function(Query)? queryBuilder,
  int limit = -1,
}) =>
    queryCollectionCount(
...
Stream<List<NeedPostRecord>> queryNeedPostRecord({
  Query Function(Query)? queryBuilder,
  int limit = -1,
  bool singleRecord = false,
}) =>
    queryCollection(
      NeedPostRecord.collection,
      NeedPostRecord.fromSnapshot,
      queryBuilder: queryBuilder,
      limit: limit,
      singleRecord: singleRecord,
    );

Future<List<NeedPostRecord>> queryNeedPostRecordOnce({
  Query Function(Query)? queryBuilder,
  int limit = -1,
  bool singleRecord = false,
}) =>
...
Future<FFFirestorePage<NeedPostRecord>> queryNeedPostRecordPage({
  Query Function(Query)? queryBuilder,
  DocumentSnapshot? nextPageMarker,
  required int pageSize,
  required bool isStream,
  required PagingController<DocumentSnapshot?, NeedPostRecord> controller,
  List<StreamSubscription?>? streamSubscriptions,
}) =>
    queryCollectionPage(
      NeedPostRecord.collection,
      ...
      if (isStream) {
        final streamSubscription =
            (page.dataStream)?.listen((List<NeedPostRecord> data) {
          for (var item in data) {
            final itemIndexes = controller.itemList!
                .asMap()
                .map((k, v) => MapEntry(v.reference.id, k));
